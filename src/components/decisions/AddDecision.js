...
import operator from "@src/data-objects/operator.json";
import decisionValidations from "@src/validations/decisionValidations";
import Tree from "@src/components/tree/tree";
import { has } from "lodash/object";
import { getNodeDepthDetails, getNodeDepth } from "@src/utils/treeutils";
import { transformTreeToRule } from "@src/utils/transform";
import { sortBy } from "lodash/collection";
import { validateAttribute } from "@src/validations/decisionValidations";
import { PLACEHOLDER } from "@src/constants/dataTypes";
...
class AddDecision extends Component {
		...
		return (
			<div className="add-decision-step">
				<div className="step1">
					...
					<ButtonGroup
						buttons={topLevelOptions}
						onConfirm={this.handleTopNode}
					/>
				</div>
				<div className="step2">
					...
					<ButtonGroup
						buttons={factsButton}
						onConfirm={this.handleChildrenNode}
					/>
				</div>
				<div className="step3">
					...
					<ButtonGroup
						...
						onConfirm={this.handleOutputPanel}
					/>
				</div>
			</div>
		);
	...
	fieldPanel() {
		...
		const placeholder =
			addAttribute.operator === "contains" || addAttribute.operator === "doesNotContain" ? PLACEHOLDER["string"] : PLACEHOLDER[attribute.type];
		...
		return (
			<Panel>
				<Tree
					treeData={node}
					count={depthCount}
					onConfirm={this.handleActiveNode}
				/>
			</Panel>
		);
	}
