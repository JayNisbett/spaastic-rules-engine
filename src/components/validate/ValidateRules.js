...
import { validateRuleset } from "@src/validations/ruleValidations";
import Loader from "@src/components/loader/loader.js";
import { ViewOutcomes } from "@src/components/attributes/ViewAttributes";

class ValidateRules extends Component {
	constructor(props) {
		super(props);
		const conditions = props.attributes.filter(attr => attr.type !== "object" && { name: attr.name, value: "" });
		this.state = { attributes: [], conditions, message: Message.NO_VALIDATION_MSG, loading: false, outcomes: [], error: false };
