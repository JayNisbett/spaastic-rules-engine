...
const NavLinks = props => {
	...
	return links.map((link, index) => (
		<ul
			...
			key={link.name + props.activeIndex}
		>
			<NavParentLink
				link={link}
...
const NavSubLink = ({ sublinks, visible, onConfirm, activeIndex }) => {
	...
	return sublinks.map(link => (
		<ul
			className={`sublink-container ${visible ? "visible" : ""}`}
			key={link}
		>
			<li
				className={`sublink ${visible ? "visible" : ""} ${active === link ? "active" : ""}`}
