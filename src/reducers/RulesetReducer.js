...
function ruleset(state = initialState, action = "") {
	switch (action.type) {
		...
		case ActionTypes.ADD_DECISION: {
			...
			return { ...state, updatedFlag: true, rulesets: replaceRulesetByIndex(state.rulesets, activeRuleSet, state.activeRuleset) };
		}
		...
		case ActionTypes.RESET_ATTRIBUTE: {
			...
			if (state.uploadedRules[state.activeRuleset] && state.uploadedRules[state.activeRuleset].attributes) {
				...
				return { ...state, rulesets: replaceRulesetByIndex(state.rulesets, activeRuleSet, state.activeRuleset) };
			}
			return { ...state };
		}
		...
		case ActionTypes.SET_FETCHED_RULE: {
			const newRule = action.payload.rule;
			const id = action.payload.id;
			// Assuming 'rulesets' is an array and these rules are supposed to be added in this array.
			return { ...state, rulesets: [...state.rulesets, { ...newRule, id }], uploadedRules: [...state.uploadedRules, { ...newRule, id }] };
		}
